...
namespace IGOR.WebShell.Controllers
{
	...
	{
		readonly IDerictory<Motherboard> Derictory = new MotherboardDerictory();
		...
		{
			return View(new ComponentModel<Motherboard> {
				List = new SelectList(Manufacturers.Items, "Id", "Name"),
				List2 = new SelectList(MemoryTypes.Items, "Id", "Name"),
				Target = new Motherboard()
			});
		}

		[HttpPost]
		public ActionResult Create(ComponentModel<Motherboard> item)
		{
		...
		{
			if (Derictory.Find(id) is Motherboard item) {
				return View(item);
			} else {
		...
		[HttpPost]
		public ActionResult Delete(Motherboard item)
		{
			Derictory.Drop(item);
		...
		[HttpPost]
		public ActionResult Edit(ComponentModel<Motherboard> item)
		{
